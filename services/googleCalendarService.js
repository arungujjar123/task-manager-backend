...
const envConfigured = requiredEnvVars.every(varName => !!process.env[varName]);

// Create OAuth2 client
const oauth2Client = envConfigured ? new google.auth.OAuth2(
      ...
      try {
        ...
        if (task.googleCalendarEventId) {
          ...
          try {
            ...
            if (error.code === 404) {
              const result = await googleCalendarService.createCalendarEvent(tokens, task);
        ...
        console.error(`Error syncing task ${task._id} to Google Calendar:`, error);
        results.push({
          taskId: task._id,
          action: 'error',
          error: error.message,
